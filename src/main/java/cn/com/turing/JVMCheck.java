...
final class JVMCheck {
    ...
    static final Map<String, JVMCheck.HotspotBug> JVM_BROKEN_HOTSPOT_VERSIONS;

    static {
        Map<String, JVMCheck.HotspotBug> bugs = new HashMap<>();

        // 1.7.0: loop optimizer bug
        bugs.put("21.0-b17",  new JVMCheck.HotspotBug("https://bugs.openjdk.java.net/browse/JDK-7070134", "-XX:-UseLoopPredicate"));
        // register allocation issues (technically only x86/amd64). This impacted update 40, 45, and 51
        bugs.put("24.0-b56",  new JVMCheck.HotspotBug("https://bugs.openjdk.java.net/browse/JDK-8024830", "-XX:-UseSuperWord"));
        bugs.put("24.45-b08", new JVMCheck.HotspotBug("https://bugs.openjdk.java.net/browse/JDK-8024830", "-XX:-UseSuperWord"));
        bugs.put("24.51-b03", new JVMCheck.HotspotBug("https://bugs.openjdk.java.net/browse/JDK-8024830", "-XX:-UseSuperWord"));
    ...
    static void check() {
        if (Boolean.parseBoolean(System.getProperty(JVM_BYPASS))) {
            ...
            JVMCheck.HotspotBug bug = JVM_BROKEN_HOTSPOT_VERSIONS.get(Constants.JVM_VERSION);
            if (bug != null) {
